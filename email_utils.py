...
class EmailService:
    ...
    def send_meeting_invite(self, meeting_data: MeetingInviteData) -> bool:
        ...
        try:
            ...
            agenda_text = "\n".join(f"• {item}" for item in meeting_data.agenda_items) if meeting_data.agenda_items else "No agenda items specified"
            
            text_content = (
                "Meeting Invitation\n\n"
                f"Hi {meeting_data.recipient_name}!\n\n"
                f"You're invited to join: {meeting_data.meeting_title}\n\n"
                f"Organization: {meeting_data.organization_name}\n"
                f"Project: {meeting_data.project_name}\n\n"
                "Details:\n"
                f"📅 Date: {meeting_data.meeting_date}\n"
                f"🕐 Time: {meeting_data.meeting_time}\n"
                f"💻 Platform: {meeting_data.meeting_platform}\n\n"
                f"Join here: {meeting_data.meeting_link}\n\n"
                "Agenda:\n"
                f"{agenda_text}\n\n"
                f"Organized by: {meeting_data.organizer_name} ({meeting_data.organizer_email})\n\n"
                "Can't attend? Please let the organizer know as soon as possible."
            )
    ...
    def send_participant_invite(self, to_email: str, project_name: str, inviter_name: str):
        ...
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
            ...
            <p>Hello,</p>
            <p>{} has invited you to join the project <b>{}</b>.</p>
            <p>Please log in to your account to accept the invitation.</p>
        </div>
        """.format(inviter_name, project_name)
    ...
    def send_role_update(self, to_email: str, project_name: str, new_role: str):
        ...
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
            ...
            <p>Hello,</p>
            <p>Your role in the project <b>{}</b> has been changed to <b>{}</b>.</p>
            <p>Please log in to see your new permissions.</p>
        </div>
        """.format(project_name, new_role)
        return self.send_email(to_email, subject, html_content)
    ...
    def send_removal_notification(self, to_email: str, project_name: str):
        ...
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
            ...
            <p>Hello,</p>
            <p>You have been removed from the project <b>{}</b>.</p>
